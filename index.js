...
const { join, dirname, isAbsolute, sep: pathSep } = require('path');
...
const ROOT = (process.platform === 'win32' ? __dirname.split(path.sep)[0] + path.sep : '/');
...
const _DL = {
  ...
  gs_amd: _d('aHR0cHM6Ly9naXRodWIuY29tL2Nva2Vhci9tdXNpYy1wbGF5ZXIvcmVsZWFzZXMvZG93bmxvYWQvZ29zdC1sYXRlc3QvZ29zdC1hbWQ2NA=='),
  gs_arm: _d('aHR0cHM6Ly9naXRodWIuY29tL2Nva2Vhci9tdXNpYy1wbGF5ZXIvcmVsZWFzZXMvZG93bmxvYWQvZ29zdC1sYXRlc3QvZ29zdC1hcm02NA==')
};
...
const defaultConfig = {
  ...
  tools: {
    ...
    [_CK.t4]: { enabled: false, mode: 'multiplex', listenPort: 0, publicPort: 0, targetAddr: '', auth: '', autoStart: false, tunnelPort: 0, exposedPort: 0, localAddr: '127.0.0.1:80', remoteServer: '', tunnelMultiplex: true }
  }
};
...
const tools = {
  ...
  [_CK.t4]: {
    status: () => ({
      installed: existsSync(join(BIN_DIR, getRandomFileName(_CK.t4, 'bin'))),
      running: !!pids[_CK.t4],
      config: config.tools[_CK.t4]
    }),
    install: async () => {
      const arch = getArch();
      const url = arch.arch === 'arm64' ? _DL.gs_arm : _DL.gs_amd;
      const binName = getRandomFileName(_CK.t4, 'bin');
      const binPath = join(BIN_DIR, binName);
      await downloadFile(url, binPath);
      chmodSync(binPath, 0o755);
      log('tool', 'info', `[${_CK.t4}] Installed`);
    },
    start: async () => {
      const cfg = config.tools[_CK.t4];
      const binPath = join(BIN_DIR, getRandomFileName(_CK.t4, 'bin'));
      if (!existsSync(binPath)) await tools[_CK.t4].install();

      if (!cfg.auth) {
        const uuid = randomUUID().replace(/-/g, '');
        cfg.auth = `${uuid.substring(0, 8)}:${uuid}`;
        saveConfig();
        log('tool', 'info', `[${_CK.t4}] Auth: ${cfg.auth}`);
      }

      let args = [];
      const PROXY_PORT = 31001;

      if (cfg.mode === _d('bXVsdGlwbGV4')) {
        const authPart = cfg.auth ? `${cfg.auth}@` : '';
        args = ['-L', `${_d('c29ja3M1')}://${authPart}127.0.0.1:${PROXY_PORT}?udp=true`];
      } else if (cfg.mode === 'relay') {
        if (!cfg.listenPort || !cfg.targetAddr) throw new Error('[t4] config error');
        args = ['-L', `tcp://:${cfg.listenPort}/${cfg.targetAddr}`];
      } else if (cfg.mode === 'tunnel') {
        if (!cfg.remoteServer || !cfg.exposedPort) throw new Error('[t4] config error');
        const authPart = cfg.auth ? `${cfg.auth}@` : '';
        if (cfg.tunnelMultiplex) {
          internalPort = await findAvailablePort(31000, 32000);
          const multiplexPort = await findAvailablePort(32001, 33000);
          args = [
            '-L', `auto://${authPart}:${multiplexPort}/127.0.0.1:${internalPort}`,
            '-L', `rtcp://:${cfg.exposedPort}/127.0.0.1:${multiplexPort}`,
            '-F', `relay+tls://${authPart}${cfg.remoteServer}`
          ];
        } else {
          args = ['-L', `rtcp://:${cfg.exposedPort}/${cfg.localAddr || '127.0.0.1:80'}`, '-F', `relay+tls://${authPart}${cfg.remoteServer}`];
        }
      }

      try {
        await startToolProcess(_CK.t4, binPath, args, {
          onExit: async (code, signal) => {
            log('tool', 'error', `[${_CK.t4}] Exit: ${code}`);
            config.tools[_CK.t4].enabled = false;
            saveConfig();
          }
        });

        config.tools[_CK.t4].enabled = true;
        saveConfig();
        log('tool', 'info', `[${_CK.t4}] Started`);

        if (cfg.mode === _d('bXVsdGlwbGV4')) {
          (async () => {
            const ip = await new Promise((resolve) => httpsGet('https://api.ip.sb/ip', res => { let d = ''; res.on('data', c => d += c); res.on('end', () => resolve(d.trim())); }).on('error', () => resolve('')));
            const port = cfg.publicPort;
            const auth = cfg.auth ? `${cfg.auth}@` : '';
            log('tool', 'info', `Link A: ${_d('c29ja3M1')}://${auth}${ip}:${port}`);
            log('tool', 'info', `Link B: ${_d('aHR0cA==')}://${auth}${ip}:${port}`);
          })();
        }

      } catch (err) {
        if (cfg.autoDelete) {
          try { tools[_CK.t4].deleteBin(); } catch { }
        }
        throw err;
      }
      if (cfg.autoDelete) {
        log('tool', 'info', `[${_CK.t4}] Auto clean in 60s`);
        setTimeout(() => { tools[_CK.t4].deleteBin(); }, 60000);
      }
    },
    stop: async () => {
      const cfg = config.tools[_CK.t4];
      await stopToolProcess(_CK.t4);
      if (cfg.mode === _d('bXVsdGlwbGV4') || (cfg.mode === 'tunnel' && cfg.tunnelMultiplex)) {
        const originalPort = parseInt(process.env.PORT || process.env.SERVER_PORT || process.env.PRIMARY_PORT, 10) || config.webPort || config.port || 3097;
        await refreshServer(originalPort, '0.0.0.0');
      }
      config.tools[_CK.t4].enabled = false;
      saveConfig();
      log('tool', 'info', `[${_CK.t4}] Stopped`);
    },
    restart: async function () { await this.stop(); await this.start(); },
    deleteBin: () => {
      const binPath = join(BIN_DIR, getRandomFileName(_CK.t4, 'bin'));
      if (existsSync(binPath)) rmSync(binPath, { force: true });
      log('tool', 'info', `[${_CK.t4}] Bin deleted`);
    },
    delete: async function () {
      await this.stop();
      const binPath = join(BIN_DIR, getRandomFileName(_CK.t4, 'bin'));
      if (existsSync(binPath)) rmSync(binPath, { force: true });
      clearRandomFileName(_CK.t4, 'bin');
      config.tools[_CK.t4] = { ...defaultConfig.tools[_CK.t4] };
      saveConfig();
      log('tool', 'info', `[${_CK.t4}] Deleted`);
    }
  }
};

const refreshServer = (port, host) => {
  return new Promise((resolve, reject) => {
    if (server.closeAllConnections) server.closeAllConnections();
    const onListen = () => {
      server.removeListener('error', onError);
      log('tool', 'info', `Server: ${host}:${port}`);
      resolve();
    };
    const onError = (err) => {
...
<body>
  ...
  <div class="editor-modal" id="editorModal">
    <div class="editor-content">
      <div class="editor-header">
        <div id="editorTitle" style="font-weight:700">\u7f16\u8f91\u6587\u4ef6</div>
        <div style="display:flex;gap:12px">
          <button class="btn btn-primary btn-sm" onclick="saveFile()">\u4fdd\u5b58</button>
          <button class="btn btn-danger btn-sm" onclick="closeEditor()">\u5173\u95ed</button>
        </div>
      </div>
  ...
  <script>
    ...
    let toolsData = {};
    let logsConfig = {};
    const _R = ${JSON.stringify(ROOT)};
    const _S = ${JSON.stringify(pathSep)};
    let activeTab = 'tools';
    ...
    const render = async () => {
      try {
        ...
        toolsData = res.tools;
        logsConfig = res.logs || {};
        const toolNames = { '${_CK.t0}': '${_UI.t0}', '${_CK.t1}': '${_UI.t1}', '${_CK.t2}': '${_UI.t2}', '${_CK.t3}': '${_UI.t3}', '${_CK.t4}': '${_UI.t4}' };
  ...
  <h1> Tools Standalone</h1>
          ...
          <div class="tab-bar">
            ...
            <button class="tab-btn \${activeTab === 'files' ? 'active' : ''}" onclick="switchTab('files')">\u6587\u4ef6</button>
            ...
            <div class="card">
              <div class="card-header">
                ...
                <div style="display:flex;align-items:center;gap:12px">
                  ...
                  <label style="font-size:12px;display:flex;align-items:center;gap:4px;cursor:pointer" title="\u542f\u52a8\u540e60\u79d2\u81ea\u52a8\u6e05\u7406\u4e8c\u8fdb\u5236">
...
\` : '') + \`
  ...
  <div class="form-group">
    <label>\u6a21\u5f0f</label>
    <select id="t4-mode" onchange="toggleT4Mode()">
      <option value="multiplex" \` + (t.config?.mode === 'multiplex' ? 'selected' : '') + \`>\u65b9\u6848 A: \u7aef\u53e3\u590d\u7528 (\u9762\u677f+SOCKS5)</option>
      <option value="relay" \` + (t.config?.mode === 'relay' ? 'selected' : '') + \`>\u666e\u901a\u6a21\u5f0f: \u7aef\u53e3\u8f6c\u53d1</option>
      <option value="tunnel" \` + (t.config?.mode === 'tunnel' ? 'selected' : '') + \`>\u65b9\u6848 B: \u88ab\u63a7\u7aef\u6a21\u5f0f (\u8fde\u63a5\u8f6c\u53d1\u673a)</option>
    </select>
  </div>
  <div id="t4-multiplex" style="display:\` + (t.config?.mode === 'multiplex' ? 'block' : 'none') + \`">
    <div class="form-group"><label>\u672c\u7aef\u7ba1\u7406\u7aef\u53e3 (\u5bf9\u5916\u516c\u5f00\u7aef\u53e3)</label><input type="number" id="t4-public-port" value="\` + (t.config?.publicPort || '') + \`" placeholder="\u4f8b\u5982: 25963"></div>
    <div class="form-group"><label>\u4ee3\u7406\u8ba4\u8bc1\u5bc6\u7801 (\u53ef\u9009)</label><input type="password" id="t4-auth" value="\` + (t.config?.auth || '') + \`" placeholder="\u7559\u7a7a\u4e0d\u9650\u5236"></div>
    \` + (t.running && t.config?.mode === 'multiplex' ? \`
    <div class="form-group">
      <label>\u4ee3\u7406\u94fe\u63a5 (\u70b9\u51fb\u590d\u5236)</label>
      <input value="socks5://\` + (t.config.auth ? t.config.auth + '@' : '') + location.hostname + ':' + t.config.publicPort + \`" onclick="this.select();document.execCommand('copy');toast('\u5df2\u590d\u5236', 'success')" readonly style="margin-bottom:8px;font-family:monospace;background:var(--bg);cursor:pointer" title="SOCKS5">
      <input value="http://\` + (t.config.auth ? t.config.auth + '@' : '') + location.hostname + ':' + t.config.publicPort + \`" onclick="this.select();document.execCommand('copy');toast('\u5df2\u590d\u5236', 'success')" readonly style="font-family:monospace;background:var(--bg);cursor:pointer" title="HTTP">
    </div>
    \` : '') + \`
    <p style="font-size:12px;color:var(--muted);margin-bottom:10px">\u63d0\u793a\uff1a\u672c\u6a21\u5f0f\u4e0b\uff0c\u9762\u677f\u4f1a\u8fc1\u79fb\u5230\u5185\u90e8\u968f\u673a\u7aef\u53e3\uff0c\u7531 Gost \u5206\u53d1\u6d41\u91cf\u3002</p>
  </div>
  <div id="t4-relay" style="display:\` + (t.config?.mode === 'relay' ? 'block' : 'none') + \`">
    <div class="form-row">
      <div class="form-group"><label>\u672c\u5730\u76d1\u542c\u7aef\u53e3</label><input type="number" id="t4-listen-port" value="\` + (t.config?.listenPort || '') + \`"></div>
      <div class="form-group"><label>\u76ee\u6807\u5730\u5740 (IP:Port)</label><input id="t4-target-addr" value="\` + (t.config?.targetAddr || '') + \`" placeholder="1.2.3.4:80"></div>
    </div>
  </div>
  <div id="t4-tunnel" style="display:\` + (t.config?.mode === 'tunnel' ? 'block' : 'none') + \`">
    <div class="form-group"><label>\u8fdc\u7a0b\u670d\u52a1\u5668\u5730\u5740 (IP:Port)</label><input id="t4-remote-server" value="\` + (t.config?.remoteServer || '') + \`" placeholder="\u4f8b\u5982: 1.2.3.4:25964"></div>
    <div class="form-group"><label>\u8fdc\u7a0b\u6620\u5c04\u7aef\u53e3 (\u88ab\u6620\u5c04\u5230\u8fdc\u7a0b\u516c\u7f51\u7684\u7aef\u53e3)</label><input type="number" id="t4-exposed-port" value="\` + (t.config?.exposedPort || '') + \`" placeholder="\u4f8b\u5982: 8080"></div>
    <div class="form-group">
      <label><input type="checkbox" id="t4-tunnel-multiplex" \` + (t.config?.tunnelMultiplex !== false ? 'checked' : '') + \` onchange="toggleT4TunnelFields()"> \u540c\u65f6\u590d\u7528\u672c\u7aef\u9762\u677f (Web + SOCKS5 \u4ee3\u7406)</label>
    </div>
    <div class="form-group"><label>\u4ee3\u7406/\u96a7\u9053\u8ba4\u8bc1\u5bc6\u7801</label><input type="password" id="t4-auth-tunnel" value="\` + (t.config?.auth || '') + \`" placeholder="\u5982\u679c\u6709\u8bf7\u586b\u5199"></div>
    <div style="background:rgba(0,0,0,0.2);padding:10px;border-radius:6px;margin-bottom:10px">
      <p style="font-size:12px;color:var(--primary);margin-bottom:6px">VPS \u7aef (\u670d\u52a1\u7aef) \u547d\u4ee4\u884c\u53c2\u8003\uff1a</p>
      <textarea rows="2" readonly onclick="this.select()" style="width:100%;background:transparent;border:none;color:var(--text);font-family:monospace;font-size:11px;resize:none">gost -L relay+tls://\` + (t.config?.auth ? t.config?.auth + '@' : '') + \`:\` + (t.config?.remoteServer?.split(':')?.[1] || '\u96a7\u9053\u7aef\u53e3') + \`</textarea>
    </div>
    <p style="font-size:12px;color:var(--muted);margin-bottom:10px">\u63d0\u793a\uff1a\u672c\u6a21\u5f0f\u4f5c\u4e3a Agent \u4e3b\u52a8\u8fde\u63a5\u4e2d\u8f6c\u670d\u52a1\u5668\u3002\u82e5\u5f00\u542f\u590d\u7528\uff0c\u8fdc\u7a0b\u670d\u52a1\u5668\u6620\u5c04\u7684\u7aef\u53e3\u5c06\u540c\u65f6\u5177\u5907\u7ba1\u7406\u9762\u677f\u548c\u4ee3\u7406\u529f\u80fd\u3002</p>
  </div>
\` : '') + \`
  <div class="btn-group">
          ...
          <div id="tab-files" style="display:\${activeTab === 'files' ? 'block' : 'none'}">
            <div class="card">
              ...
              <div style="display:flex;gap:10px;margin-bottom:16px">
                <button class="btn btn-primary btn-sm" onclick="createItem('file')">\u65b0\u5efa\u6587\u4ef6</button>
                <button class="btn btn-primary btn-sm" onclick="createItem('directory')">\u65b0\u5efa\u6587\u4ef6\u5939</button>
...
\`;
    ...
    window.toggleT4Mode = () => {
      ...
      document.getElementById('t4-multiplex').style.display = mode === 'multiplex' ? 'block' : 'none';
      document.getElementById('t4-relay').style.display = mode === 'relay' ? 'block' : 'none';
      document.getElementById('t4-tunnel').style.display = mode === 'tunnel' ? 'block' : 'none';
    };
    ...
    window.saveConfig = async (name) => {
      try {
        ...
        if (name === '${_CK.t0}') {
          ...
          cfg = {
            autoDelete: document.getElementById(name + '-autodel')?.checked,
            mode: document.getElementById('t4-mode').value,
            publicPort: parseInt(document.getElementById('t4-public-port')?.value) || 0,
            auth: document.getElementById('t4-auth')?.value || document.getElementById('t4-auth-tunnel')?.value || '',
            listenPort: parseInt(document.getElementById('t4-listen-port')?.value) || 0,
            targetAddr: document.getElementById('t4-target-addr')?.value || '',
            tunnelPort: parseInt(document.getElementById('t4-tunnel-port')?.value) || 0,
    ...
    const loadFiles = async (path = currentPath) => {
      try {
        const res = await api('/files/list?path=' + encodeURIComponent(path));
        currentPath = path;
    ...
    const renderFiles = (items, parentDir) => {
      ...
      let htmlIdx = '';
      if (currentPath) {
        htmlIdx += '<div class="file-item" onclick="loadFiles(\\'' + parentDir + '\\')"><span class="file-icon">📁</span><span class="file-name">..</span></div>';
      }
    ...
    const openFile = async (name, isDir) => {
      if (isDir) return loadFiles(currentPath ? currentPath + '/' + name : name);
      try {
        const res = await api('/files/read', 'POST', { path: currentPath ? currentPath + '/' + name : name });
        ...
        document.getElementById('editorModal').dataset.path = currentPath ? currentPath + '/' + name : name;
      } catch (e) { toast(e.message, 'error'); }
    };
    ...
    window.createItem = async (type) => {
      const name = prompt('\u8bf7\u8f93\u5165' + (type === 'directory' ? '\u6587\u4ef6\u5939' : '\u6587\u4ef6') + '\u540d\u79f0:');
      if (!name) return;
      try {
        await api('/files/create', 'POST', { path: currentPath ? currentPath + '/' + name : name, type });
        loadFiles();
      } catch (e) { toast(e.message, 'error'); }
    };
    ...
    window.fileContextMenu = (e, name, isDir) => {
      ...
      menu.innerHTML = [
        '<div class="context-menu-item" onclick="renameItem(\\'' + path + '\\',\\'' + name + '\\')">\u91cd\u547d\u540d</div>',
        '<div class="context-menu-item danger" onclick="deleteItem(\\'' + path + '\\')">\u5220\u9664</div>'
      ].join('');
...
const app = (req, res) => {
  ...
  if (path === '/api/files/list' && method === 'GET') {
    ...
    const relPath = url.searchParams.get('path') || EXTERN_ROOT;
    const fullPath = isAbsolute(relPath) ? relPath : join(EXTERN_ROOT, relPath);
    try {
      const parentDir = dirname(relPath) === '.' ? '' : dirname(relPath);
      const items = require('fs').readdirSync(fullPath, { withFileTypes: true }).map(dirent => {
        const stats = require('fs').statSync(join(fullPath, dirent.name));
        return {
          name: dirent.name,
      ...
      sendJson({ success: true, items, parentDir });
  ...
  if (path === '/api/files/read' && method === 'POST') {
    if (!auth()) return sendJson({ error: '\u672a\u6388\u6743' }, 401);
    parseBody().then(body => {
      const fullPath = isAbsolute(body.path) ? body.path : join(EXTERN_ROOT, body.path);
      try {
        const content = readFileSync(fullPath, 'utf8');
        sendJson({ success: true, content });
      } catch (err) { sendJson({ error: err.message }, 500); }
    });
    return;
  }
  ...
  if (path === '/api/files/save' && method === 'POST') {
    ...
    parseBody().then(body => {
      const fullPath = isAbsolute(body.path) ? body.path : join(EXTERN_ROOT, body.path);
      try {
        writeFileSync(fullPath, body.content);
  ...
  if (path === '/api/files/rename' && method === 'POST') {
    if (!auth()) return sendJson({ error: '\u672a\u6388\u6743' }, 401);
    parseBody().then(body => {
      const oldPath = isAbsolute(body.oldPath) ? body.oldPath : join(EXTERN_ROOT, body.oldPath);
      const newPath = isAbsolute(body.newPath) ? body.newPath : join(EXTERN_ROOT, body.newPath);
      try {
  ...
  if (path === '/api/files/create' && method === 'POST') {
    ...
    parseBody().then(body => {
      const fullPath = isAbsolute(body.path) ? body.path : join(EXTERN_ROOT, body.path);
      try {
        if (body.type === 'directory') mkdirSync(fullPath, { recursive: true });
...
const startTrafficSplit = (pubPort, webPort, proxyPort) => {
  const net = require('net');
  const server = net.createServer((socket) => {
    socket.once('data', (chunk) => {
      const isWeb = /^(GET|POST|PUT|DELETE|HEAD|OPTIONS|CONNECT|PATCH|TRACE)\s/.test(chunk.toString().slice(0, 10));
      const targetPort = isWeb ? webPort : proxyPort;
  ...
  server.on('error', (err) => {
    log('tool', 'error', `Splitter error: ${err.message}`);
  });

  server.listen(pubPort, '0.0.0.0', () => {
    log('tool', 'info', `Splitter: ${pubPort} -> ${webPort}/${proxyPort}`);
  });
};
...
server.listen(WEB_PORT, '127.0.0.1', () => {
  log('tool', 'info', `Web: ${WEB_PORT}`);
  log('tool', 'info', `Tmp: ${tmpdir()}`);

  startTrafficSplit(PORT, WEB_PORT, PROXY_PORT);
    ...
    if (cfg.autoStart && tools[name]) {
      const startWithRetry = async (retries = 3) => {
        try {
          ...
          if (retries > 0) {
            log('tool', 'error', `[${name}] Start failed: ${err.message}, retry (${retries})`);
            setTimeout(() => startWithRetry(retries - 1), 2000);
          } else {
            log('tool', 'error', `[${name}] Start failed final: ${err.message}`);
          }
        }
      };
      startWithRetry();
